...
import MSFTPivotBase, { PivotBaseHandledProps, PivotBaseUnhandledProps } from "./pivot-base";
...
describe("pivot base", (): void => {
    test("should have a displayName that matches the component name", () => {
        expect(`${DisplayNamePrefix}${(MSFTPivotBase as any).name}`).toBe(
            MSFTPivotBase.displayName
        );
    });

    test("should not throw if managedClasses are not provided", () => {
        expect(() => {
            shallow(<MSFTPivotBase label={"foo"} items={detailPivotItemData} />);
        }).not.toThrow();
    });

    test("should accept unhandledProps", (): void => {
        const handledProps: PivotBaseHandledProps = {
            label: "foo",
            items: detailPivotItemData,
        };

        const unhandledProps: PivotBaseUnhandledProps = {
            "aria-hidden": true,
        };

        const props: PivotBaseHandledProps & PivotBaseUnhandledProps = {
            ...handledProps,
            ...unhandledProps,
        };

        const rendered: any = mount(<MSFTPivotBase {...props} />);

        expect(rendered.prop("aria-hidden")).toEqual(true);
    });

    test("should not error with empty item data", () => {
        const rendered: any = mount(<MSFTPivotBase label={"foo"} items={[]} />);
    ...
    test("should render with correct tab set as active", () => {
        const rendered: any = mount(
            <MSFTPivotBase label={"foo"} items={[]} activeId={id2} />
        );
    ...
    test("should not throw if items return no functions", () => {
        expect(() => {
            shallow(<MSFTPivotBase label={"foo"} items={detailNull} />);
        }).not.toThrow();
    });

    test("should move to the second pivot item on second pivot item click", () => {
        const rendered: any = mount(
            <MSFTPivotBase label={"foo"} items={detailPivotItemData} />
        );
